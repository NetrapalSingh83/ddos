...
#define SHM_NAME "/udp_shm"
#define SHM_SIZE 1024
#define THREAD_STACK_SIZE 16 * 1024  // 16KB stack per thread
...
int main(int argc, char *argv[]) {
    ...
    // Setup shared memory for data tracking
    int shm_fd = shm_open(SHM_NAME, O_CREAT | O_RDWR, 0666);
    ftruncate(shm_fd, SHM_SIZE);
    total_data_sent = mmap(0, SHM_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, shm_fd, 0);
    *total_data_sent = 0;
    ...
    for (int i = 0; i < thread_count; i++) {
        ...
        if (pthread_create(&threads[i], &attr, udp_flood, &params[i]) != 0) {
    ...
    munmap(total_data_sent, SHM_SIZE);
    close(shm_fd);
    shm_unlink(SHM_NAME);
